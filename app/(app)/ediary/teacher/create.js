...
export default function App() {
    ...
    const receptientsDropdown = (
        <ScrollView style={{position:'absolute', width:'100%', maxHeight:300, top:80, paddingVertical:6, zIndex:2, backgroundColor:'#fff', borderColor:'#ccc', borderWidth:1, borderBottomRightRadius:4, borderBottomLeftRadius:4}}>
            ...
            {filteredRecipiens?.length == 0 ? (
                ...
                <>
                    <View style={{display:'flex', flexDirection:'row', alignItems:'center', marginBottom:6, borderBottomWidth:1, borderBottomColor:'#ccc'}}>
                        <Checkbox
                            status={filteredRecipiens.length === selectedReceptients.length ? 'checked' : 'unchecked'}
                            onPress={() => selectedReceptients.length === filteredRecipiens.length ? setSelectedReceptients([]) : setSelectedReceptients(filteredRecipiens)}
                        />
                        <Text style={{fontWeight:'600'}}>Select All</Text>
                    </View>
    ...
    const submitHandler = async () => {
        ...
        try {
            ...
            router.push({pathname:'/ediary/teacher', params:{isSubmitted:true}});
        ...
        <View style={{height:'100%', alignItems:'center'}}>
            <View style={{width:'100%', height:120, display:'flex', flexDirection:'row', alignItems:'flex-end', justifyContent:'space-between', paddingHorizontal:10, paddingBottom:30, backgroundColor:'#0094DA', borderBottomRightRadius:40, borderBottomLeftRadius:40}}>
                <View style={{display:'flex', flexDirection:'row', alignItems:'center', gap:20}}>
                    <TouchableOpacity
                        onPress={() => router.push('/ediary/teacher')}
                    >
