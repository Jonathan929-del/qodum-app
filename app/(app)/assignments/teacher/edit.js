...
import {TextInput as PaperTextInput, ActivityIndicator, Icon, Switch} from 'react-native-paper';
...
const CreateAssignment = () => {
    ...
    const onSubmit = async data => {
        ...
        try {
            ...
            if(assignment.attachment.split('/')[4] !== selectedFile?.assets[0]?.name){
                pdfUploadResponse = await uploadFileToS3();
            };

            // Api call
            const link = `${process.env.EXPO_PUBLIC_API_URL}/assignments/edit`;
            const params = {
                id:assignment._id,
    ...
    useEffect(() => {
        ...
        const fetcher = async () => {
            ...
            // Setting values
            setSelectedSubject({label:assignment.subject, value:assignment.subject.toLowerCase()});
        ...
        <View style={{height:'100%', alignItems:'center'}}>
            <View style={{width:'100%', height:120, display:'flex', flexDirection:'row', alignItems:'flex-end', justifyContent:'space-between', paddingHorizontal:10, paddingBottom:30, backgroundColor:'#0094DA', borderBottomRightRadius:40, borderBottomLeftRadius:40}}>
                <View style={{display:'flex', flexDirection:'row', alignItems:'center', gap:20}}>
                    ...
                    <Text style={{textAlign:'center', fontSize:18, color:'#fff', fontWeight:'900'}}>Edit Assignment</Text>
                </View>
            </View>
                        ...
                        <>
                            ...
                            <View style={{gap:6}}>
                                ...
                                <Dropdown
                                    ...
                                    onChange={item => {setSelectedSubject({label:item.label, value:item.value});setStates({states, errors:{...states.errors, subject:!item.label  ? '*Please select a subject' : ''}})}}
