...
const App = () => {
    ...
    useEffect(() => {
        const fetcher = async () => {
            ...
            const classStudnetsRes = await axios.post(classStudnetsLink, {class_name:a.class_name});
            console.log(classStudnetsRes.data);
        };
        fetcher();
    }, []);
        ...
        <View style={{height:'100%'}}>
            ...
            <ScrollView style={{width:'100%'}} contentContainerStyle={{height:'100%', alignItems:'center', paddingVertical:30, gap:30}}>
                ...
                <View style={{width:'80%', gap:10}}>
                    ...
                    {/* Submitted answers */}
                    <View style={{gap:20}}>
                        {students.filter(s => a.submitted_assignments.map(sa => sa.student.name).includes(s.student.name)).map(s => (
                            <Card style={{width:'100%', height:120, borderRadius:10, backgroundColor:'#fff'}} key={s._id}>
                                <View style={{height:'100%', display:'flex', flexDirection:'column', justifyContent:'space-between'}}>
                                    ...
                                    <View style={{display:'flex', flex:1, flexDirection:'column', justifyContent:'center', gap:4, padding:10}}>
                                        ...
                                        <View style={{display:'flex', flexDirection:'row', alignItems:'center', gap:2}}>
                                            ...
                                            <Text style={{fontSize:13, color:'gray'}}>{students.filter(s => a.submitted_assignments.map(sa => sa.student.name).includes(s.student.name)).indexOf(s) + 1}</Text>
                                        </View>
                                        <View style={{display:'flex', flexDirection:'row', alignItems:'center', gap:2}}>
                                    ...
                                    <View style={{width:'100%', display:'flex', flexDirection:'row', backgroundColor:'#DAE0EF', borderBottomLeftRadius:10, borderBottomRightRadius:10}}>
                                        <TouchableOpacity
                                            onPress={() => router.push({pathname:'/assignments/student/answer'})}
                                            style={{flex:1, height:'100%', display:'flex', flexDirection:'row', alignItems:'center', justifyContent:'center', gap:6, paddingVertical:10, borderBottomLeftRadius:10, borderRightColor:'#fff', borderRightWidth:1.5}}
                                        >
                                            <Icon source='eye' color='#3C5EAB' size={20}/>
                                            <Text style={{color:'#3C5EAB'}}>Add Feedback</Text>
                                        </TouchableOpacity>
                                    </View>
